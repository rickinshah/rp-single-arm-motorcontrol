...
bool ModbusAsciiToBytes(const uint8_t* ascii_response, uint16_t ascii_len, uint16_t max_byte_len,
                        uint8_t* byte_response, uint16_t* out_byte_len) {
    ...
    *out_byte_len        = 0;
    ...
    if (ascii_response[0] != ':' || ascii_response[ascii_len - 2] != '\r' ||
        ascii_response[ascii_len - 1] != '\n')
        return false;

    while ((ascii_index + 1) < (ascii_len - 2) &&
           *out_byte_len < max_byte_len) {  // remove '\r' and '\n'
        uint8_t byte;
